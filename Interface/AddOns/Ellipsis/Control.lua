...
do ------------------------
	...
	function Ellipsis:COMBAT_LOG_EVENT_UNFILTERED(timestamp, subEvent, _, sourceGUID, sourceName, sourceFlags, _, destGUID, destName, destFlags, _, arg1)
...
function Ellipsis:UNIT_AURA(unitTag)
	...
	local spellName, _, _, stackCount, _, duration, expireTime, unitCaster, _, _, spellID = UnitAura(unitTag, 1, filter)
	...
	while (spellName) do -- only scanning our auras on target
		...
		index = index + 1
		spellName, _, _, stackCount, _, duration, expireTime, unitCaster, _, _, spellID = UnitAura(unitTag, index, filter)
	end

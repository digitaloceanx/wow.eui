...
function AS:Embed_Toggle(Message)
	...
	if Message and AS:CheckOption('EmbedSystemMessage') then
		local Message = format(ASL["Main: '%s'"], AS:CheckOption('EmbedMain'))
		if AS:CheckOption('EmbedSystemDual') then Message = format(ASL["Left: '%s' | Right: '%s'"], AS:CheckOption('EmbedLeft'), AS:CheckOption('EmbedRight')) end
		AS:Print(format(ASL['Embed System: - %s'], Message))
	end
end

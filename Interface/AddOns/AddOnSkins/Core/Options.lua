...
function AS:GetOptions()
				...
				args = {
					...
					EmbedSystem = {
						...
						set = function(info, value)
							AS:SetOption(info[#info], value);
							AS:Embed_Check();
							if Skada and (not value) then Skada.db:ResetProfile(); end
						end,
					},
					EmbedMain = {
						type = 'select',
					...
					EmbedSystemDual = {
						...
						set = function(info, value)
							AS:SetOption(info[#info], value);
							AS:Embed_Check();
							if not value then SkadaDB=nil;ElvUI[1]:StaticPopup_Show('CONFIG_RL'); end
						end,						
					},
					EmbedLeft = {
						type = 'select',					
	...
	if IsAddOnLoaded("ElvUI") then
		...
		Options.args.about.args.resetsettings.func = function() ElvUI[1].global.addonskins = CopyTable(Defaults) end
	end

...
local NUM_BAG_FRAMES, KEYRING_CONTAINER = NUM_BAG_FRAMES, KEYRING_CONTAINER
...
function E:RematchJournalMove()
    ...
    RematchJournal:EnableMouse(true)
    RematchJournal:SetMovable(true)
    RematchJournal:SetClampedToScreen(false)
    addon:HookScript(RematchJournal, "OnMouseDown", MouseDownHandler)
    addon:HookScript(RematchJournal, "OnMouseUp", MouseUpHandler)
    hooked["RematchJournal"] = true
end

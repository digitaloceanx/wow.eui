    ...
    for _, spellid in ipairs(active_spells) do
        ...
        if spellname then
            if debuffs[spellid] then
                name, _, icon, count, _, duration, expirationTime, caster = UnitDebuff(unitid, spellname)
            else
                name, _, icon, count, _, duration, expirationTime, caster = UnitBuff(unitid, spellname)
            end
        end
